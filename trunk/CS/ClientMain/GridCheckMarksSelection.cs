...
namespace ClientMain
{
    ...
    public class GridCheckMarksSelection
    {
        ...
        protected ArrayList selectionRowIndex;
        GridColumn column;
        ...
        public GridCheckMarksSelection()
        {
            ...
            selectionRowIndex = new ArrayList();
        }
        ...
        {
            return Convert.ToInt32(selectionRowIndex[index].ToString());
        }
        public void ClearSelection()
        {
            selection.Clear();
            selectionRowIndex.Clear();
            Invalidate();
        }
...
DevExpress.Utils.Drawing.GraphicsCache(g), r);
        ...
        void SelectRow(int rowHandle, bool select, bool invalidate)
        {
            ...
            int DSIndex = _view.GetDataSourceRowIndex(rowHandle);
            
            if (select)
            {
                selection.Add(row);
                selectionRowIndex.Add(DSIndex);
            }
            else
            {
                selection.Remove(row);
                selectionRowIndex.Remove(DSIndex);
            }
            if (invalidate)
            {
                Invalidate();
