...
{
    public partial class XSHZDTotalReport : DevExpress.XtraReports.UI.XtraReport
    {
        public XSHZDTotalReport(string strXSHZDID, string strXSHZDH, bool fgBranch)
        {
            string sql = "";
            string sqlMinor = "";
            InitializeComponent();
            if (fgBranch)
            {
                sql = "select sum(a.fxflsl1) as sumfxflsl1, sum(a.fxflje1) as sumfxflje1, "
                    ...
                    + "sum(a.hjje) as sumhjje, sum(a.xszksl) as sumxszksl, sum(a.xszkje) as sumxszkje, sum(a.jshj) as sumjshj "
                    + "from VIEW_JC_C_XSHZD a where a.xshzdid in (" + strXSHZDID + ")";

                sqlMinor = "select a.ywbmmc, a.czyxm, a.zdrq from VIEW_JC_C_XSHZD a where a.xshzdid in (" + strXSHZDID + ")";
            }
            else
            {
                sql = "select sum(a.fxflsl1) as sumfxflsl1, sum(a.fxflje1) as sumfxflje1, "
                    ...
                    + "sum(a.fxflsl10) as sumfxflsl10, sum(a.fxflje10) as sumfxflje10, sum(a.hjsl) as sumhjsl, "
                    + "sum(a.hjje) as sumhjje, sum(a.xszksl) as sumxszksl, sum(a.xszkje) as sumxszkje, sum(a.jshj) as sumjshj "
                    + "from VIEW_JT_C_XSHZD a where a.xshzdid in (" + strXSHZDID + ")";
            ...
            string strYWBMMC = dsMinor.Tables[0].Rows[0]["YWBMMC"].ToString();
            ...
            new DevExpress.XtraReports.UI.XRBinding("Text", null, "sumxszksl")});

            xrTableCell56.DataBindings.AddRange(new DevExpress.XtraReports.UI.XRBinding[] {
            new DevExpress.XtraReports.UI.XRBinding("Text", null, "sumxszkje", "{0:c2}")});
